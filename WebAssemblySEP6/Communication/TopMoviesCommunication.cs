...
namespace WebAssemblySEP6.Communication
{
    ...
    public class TopMovieCommunication : ITopMoviesCommunication
    {
        ...
        private async Task<HttpResponseTopMovies> GetTopMovies()
        {
            ...
            {
                throw new Exception(
                    $"Error: '{responseMessage.StatusCode}' - {await responseMessage.Content.ReadAsStringAsync()} - when calling GetMovieAsync");
            }

            var responseStream = await responseMessage.Content.ReadAsStreamAsync();

            var httpResponse = JsonSerializer.Deserialize<HttpResponseTopMovies>(responseStream,
